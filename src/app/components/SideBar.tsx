  ...
  return (
    <section className="w-50 fixed top-0 bottom-0 left-0 right-auto overflow-y-auto bg-white shadow-(--shadow) py-4 px-4 border-r border-r-gray-100">
      ...
      <div>
        ...
        <p className="text-xs text-black font-semibold">{user?.email ?? "<Not found>"}</p>
      </div>
      <button className="interact text-sm border-blue-300 hover:border-blue-500 mt-2 text-blue-500 py-0.5 w-full" onClick={logout}>
        Logout
      </button>
    </section>
  );
